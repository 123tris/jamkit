...
### Changed
- **The menu is split into one UXML per view.** `JamKitMenu.uxml` is now a thin composition root that instances `JamKitStartMenu.uxml`, `JamKitPauseMenu.uxml`, and `JamKitSettingsMenu.uxml` via `<ui:Template>`/`<ui:Instance>`, so each view opens on its own in UI Builder. `MenuController` is unchanged — it still takes one `VisualTreeAsset` and finds each view by name with a recursive query, so the `<ui:Instance>` container between root and view is transparent to it. `New Jam Project` copies all four documents (relative `src`, so they must stay siblings); a new `.jk-instance` USS rule fills each instance to `jk-root` and the instances are `picking-mode="Ignore"` so the always-present containers never eat clicks. **Migration:** existing projects keep their edited `JamKitMenu.uxml` (never overwritten); to adopt the split, re-run the wizard in a project without a customized menu, or hand-split the monolithic file.
